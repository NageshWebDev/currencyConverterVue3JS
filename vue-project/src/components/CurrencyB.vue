...
const { updateCurrencyB, updateCodeB, focusOnBActive } = CurrencyStore();
...
function onClickHandler(code, countryCurrency, icon) {
    ...
    updateCodeB(code);
    updateCurrencyB(currencyAmountB.value, true);
}
...
<template>
    <section class="p-5">
        <input type="tex" placeholder="Enter Amount" class="styleTextFiled" v-model="currencyAmountB"
            @focus="focusOnBActive" />
            ...
            <div v-show="displayCurrencyList" class=styleCurrencyList>
                <button v-for="currency in currencies"
                    ...
                    class="styleCurrencyBtn" :key="currency.countryCode"><span class="styleCurrencyIcon">{{
                        currency.currencyIcon }}</span> <span>{{ currency.countryName }} </span></button>
            </div>
        </div>
    </section>
</template>